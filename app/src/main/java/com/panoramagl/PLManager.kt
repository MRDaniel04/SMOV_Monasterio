...
// Define tu multiplicador de velocidad


// Solo aplicamos la velocidad extra si estamos en modo "scroll directo"
// y el dedo se está moviendo.
            if (!mIsAcceleratedTouchScrollingEnabled && mTouchStatus == PLTouchStatus.PLTouchStatusMoved) {

                // 1. Calcula el delta (diferencia) real del movimiento
                val deltaX = mEndPoint!!.x - mStartPoint!!.x
                val deltaY = mEndPoint!!.y - mStartPoint!!.y

                // 2. Calcula el punto final "amplificado" usando un punto auxiliar
                //    (No modificamos el mEndPoint real)
                val ampEndX = mStartPoint!!.x + deltaX * speedMultiplier
                val ampEndY = mStartPoint!!.y + deltaY * speedMultiplier

                // 3. Usamos un auxiliar para pasarle la rotación exagerada a la cámara
                auxiliarEndPoint!!.setValues(ampEndX, ampEndY)
                mPanorama!!.camera.rotate(this, mStartPoint, auxiliarEndPoint)

            } else {
                // 4. Si no, usa el comportamiento normal
